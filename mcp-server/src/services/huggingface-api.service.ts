/**
 * Hugging Face API Service for DeepSeek R1 via Novita Provider
 * Сервис для работы с моделью DeepSeek R1 через Hugging Face API с провайдером Novita
 */
...
export class HuggingFaceApiService {
  ...
  private async testConnection(): Promise<void> {
    try {
      ...
      throw new Error(`Failed to connect to HuggingFace API: ${(error as Error).message}`);
    }
  }
