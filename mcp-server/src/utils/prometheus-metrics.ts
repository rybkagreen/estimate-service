...
import { register, Registry, Histogram, Counter, Gauge } from 'prom-client';
 ...
 */
export function startModelCall(model: string, provider: string, operation: string): ModelCallMetrics {
 ...
 */
export function endModelCall(metrics: ModelCallMetrics, success: boolean, cached: boolean = false): void {
 ...
 */
export function recordModelError(model: string, provider: string, operation: string, errorType: string): void {
 ...
 * Record token usage
 */
export function recordTokenUsage(model: string, provider: string, inputTokens: number, outputTokens: number): void {
  if (inputTokens > 0) {
 ...
 */
export function updateModelAvailability(model: string, provider: string, available: boolean): void {
 ...
 */
function updateCacheHitRate(model: string, provider: string): void {
  // Get current hit and miss counts
  const hits = (metricsRegistry.getSingleMetric('ai_cache_hits') as Counter<string>);
  const misses = (metricsRegistry.getSingleMetric('ai_cache_misses') as Counter<string>);
